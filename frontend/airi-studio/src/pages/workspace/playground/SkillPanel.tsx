                        ...
                        {plugins.length > 0 ? (
                            plugins.map((plugin, idx) => (
                                <div key={idx} style={{ display: 'flex', alignItems: 'center', marginBottom: idx < plugins.length - 1 ? 12 : 0 }}>
                                    <img src={plugin.icon || "https://g1.itc.cn/msfe-pch-prod/300000000000/assets/images/a5df49ba69.png"} alt={plugin.name} style={{ width: 32, height: 32, marginRight: 12, borderRadius: 4 }} />
                                    <div>
                                        <Title level="h6" style={{margin: 0}}>{plugin.name || '未命名插件'}</Title>
                                        <Paragraph style={{ margin: 0, color: '#888', fontSize: 12 }}>{plugin.description || '暂无描述'}</Paragraph>
                                    </div>
                                </div>
                            ))
                        ) : (
