/* tslint:disable */
/* eslint-disable */
/*
 * Autogenerated by @creditkarma/thrift-typescript v3.7.6
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
*/
import * as thrift from "@creditkarma/thrift-server-core";
import * as Section from "./Section";
import * as __ROOT_NAMESPACE__ from "../../base";
export interface IClearConversationApiResponse {
    code?: thrift.Int64;
    msg?: string;
    data?: Section.ISection;
    BaseResp?: __ROOT_NAMESPACE__.IBaseResp;
}
export interface IClearConversationApiResponseArgs {
    code?: number | string | thrift.Int64;
    msg?: string;
    data?: Section.ISectionArgs;
    BaseResp?: __ROOT_NAMESPACE__.IBaseRespArgs;
}
export const ClearConversationApiResponseCodec: thrift.IStructCodec<IClearConversationApiResponseArgs, IClearConversationApiResponse> = {
    encode(args: IClearConversationApiResponseArgs, output: thrift.TProtocol): void {
        const obj: any = {
            code: (typeof args.code === "number" ? new thrift.Int64(args.code) : typeof args.code === "string" ? thrift.Int64.fromDecimalString(args.code) : args.code),
            msg: args.msg,
            data: args.data,
            BaseResp: args.BaseResp
        };
        output.writeStructBegin("ClearConversationApiResponse");
        if (obj.code != null) {
            output.writeFieldBegin("code", thrift.TType.I64, 1);
            output.writeI64((typeof obj.code === "number" ? new thrift.Int64(obj.code) : typeof obj.code === "string" ? thrift.Int64.fromDecimalString(obj.code) : obj.code));
            output.writeFieldEnd();
        }
        if (obj.msg != null) {
            output.writeFieldBegin("msg", thrift.TType.STRING, 2);
            output.writeString(obj.msg);
            output.writeFieldEnd();
        }
        if (obj.data != null) {
            output.writeFieldBegin("data", thrift.TType.STRUCT, 3);
            Section.SectionCodec.encode(obj.data, output);
            output.writeFieldEnd();
        }
        if (obj.BaseResp != null) {
            output.writeFieldBegin("BaseResp", thrift.TType.STRUCT, 255);
            __ROOT_NAMESPACE__.BaseRespCodec.encode(obj.BaseResp, output);
            output.writeFieldEnd();
        }
        output.writeFieldStop();
        output.writeStructEnd();
        return;
    },
    decode(input: thrift.TProtocol): IClearConversationApiResponse {
        let _args: any = {};
        input.readStructBegin();
        while (true) {
            const ret: thrift.IThriftField = input.readFieldBegin();
            const fieldType: thrift.TType = ret.fieldType;
            const fieldId: number = ret.fieldId;
            if (fieldType === thrift.TType.STOP) {
                break;
            }
            switch (fieldId) {
                case 1:
                    if (fieldType === thrift.TType.I64) {
                        const value_1: thrift.Int64 = input.readI64();
                        _args.code = value_1;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 2:
                    if (fieldType === thrift.TType.STRING) {
                        const value_2: string = input.readString();
                        _args.msg = value_2;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 3:
                    if (fieldType === thrift.TType.STRUCT) {
                        const value_3: Section.ISection = Section.SectionCodec.decode(input);
                        _args.data = value_3;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 255:
                    if (fieldType === thrift.TType.STRUCT) {
                        const value_4: __ROOT_NAMESPACE__.IBaseResp = __ROOT_NAMESPACE__.BaseRespCodec.decode(input);
                        _args.BaseResp = value_4;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                default: {
                    input.skip(fieldType);
                }
            }
            input.readFieldEnd();
        }
        input.readStructEnd();
        return {
            code: _args.code,
            msg: _args.msg,
            data: _args.data,
            BaseResp: _args.BaseResp
        };
    }
};
export class ClearConversationApiResponse extends thrift.StructLike implements IClearConversationApiResponse {
    public code?: thrift.Int64;
    public msg?: string;
    public data?: Section.ISection;
    public BaseResp?: __ROOT_NAMESPACE__.IBaseResp;
    public readonly _annotations: thrift.IThriftAnnotations = {};
    public readonly _fieldAnnotations: thrift.IFieldAnnotations = {
        code: {
            'api.body': "code",
            'agw.key': "code"
        },
        msg: {
            'api.body': "msg",
            'agw.key': "msg"
        },
        data: {
            'api.body': "data",
            'agw.key': "data"
        }
    };
    constructor(args: IClearConversationApiResponseArgs = {}) {
        super();
        if (args.code != null) {
            const value_5: thrift.Int64 = (typeof args.code === "number" ? new thrift.Int64(args.code) : typeof args.code === "string" ? thrift.Int64.fromDecimalString(args.code) : args.code);
            this.code = value_5;
        }
        if (args.msg != null) {
            const value_6: string = args.msg;
            this.msg = value_6;
        }
        if (args.data != null) {
            const value_7: Section.ISection = new Section.Section(args.data);
            this.data = value_7;
        }
        if (args.BaseResp != null) {
            const value_8: __ROOT_NAMESPACE__.IBaseResp = new __ROOT_NAMESPACE__.BaseResp(args.BaseResp);
            this.BaseResp = value_8;
        }
    }
    public static read(input: thrift.TProtocol): ClearConversationApiResponse {
        return new ClearConversationApiResponse(ClearConversationApiResponseCodec.decode(input));
    }
    public static write(args: IClearConversationApiResponseArgs, output: thrift.TProtocol): void {
        return ClearConversationApiResponseCodec.encode(args, output);
    }
    public write(output: thrift.TProtocol): void {
        return ClearConversationApiResponseCodec.encode(this, output);
    }
}
