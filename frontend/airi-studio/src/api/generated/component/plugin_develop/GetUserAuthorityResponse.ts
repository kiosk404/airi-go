/* tslint:disable */
/* eslint-disable */
/*
 * Autogenerated by @creditkarma/thrift-typescript v3.7.6
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
*/
import * as thrift from "@creditkarma/thrift-server-core";
import * as __ROOT_NAMESPACE__ from "./";
export interface IGetUserAuthorityResponse {
    code: number;
    msg: string;
    data?: __ROOT_NAMESPACE__.IGetUserAuthorityData;
    BaseResp?: __ROOT_NAMESPACE__.IBaseResp;
}
export interface IGetUserAuthorityResponseArgs {
    code: number;
    msg: string;
    data?: __ROOT_NAMESPACE__.IGetUserAuthorityDataArgs;
    BaseResp?: __ROOT_NAMESPACE__.IBaseRespArgs;
}
export const GetUserAuthorityResponseCodec: thrift.IStructCodec<IGetUserAuthorityResponseArgs, IGetUserAuthorityResponse> = {
    encode(args: IGetUserAuthorityResponseArgs, output: thrift.TProtocol): void {
        const obj: any = {
            code: args.code,
            msg: args.msg,
            data: args.data,
            BaseResp: args.BaseResp
        };
        output.writeStructBegin("GetUserAuthorityResponse");
        if (obj.code != null) {
            output.writeFieldBegin("code", thrift.TType.I32, 1);
            output.writeI32(obj.code);
            output.writeFieldEnd();
        }
        else {
            throw new thrift.TProtocolException(thrift.TProtocolExceptionType.UNKNOWN, "Required field[code] is unset!");
        }
        if (obj.msg != null) {
            output.writeFieldBegin("msg", thrift.TType.STRING, 2);
            output.writeString(obj.msg);
            output.writeFieldEnd();
        }
        else {
            throw new thrift.TProtocolException(thrift.TProtocolExceptionType.UNKNOWN, "Required field[msg] is unset!");
        }
        if (obj.data != null) {
            output.writeFieldBegin("data", thrift.TType.STRUCT, 3);
            __ROOT_NAMESPACE__.GetUserAuthorityDataCodec.encode(obj.data, output);
            output.writeFieldEnd();
        }
        if (obj.BaseResp != null) {
            output.writeFieldBegin("BaseResp", thrift.TType.STRUCT, 255);
            __ROOT_NAMESPACE__.BaseRespCodec.encode(obj.BaseResp, output);
            output.writeFieldEnd();
        }
        output.writeFieldStop();
        output.writeStructEnd();
        return;
    },
    decode(input: thrift.TProtocol): IGetUserAuthorityResponse {
        let _args: any = {};
        input.readStructBegin();
        while (true) {
            const ret: thrift.IThriftField = input.readFieldBegin();
            const fieldType: thrift.TType = ret.fieldType;
            const fieldId: number = ret.fieldId;
            if (fieldType === thrift.TType.STOP) {
                break;
            }
            switch (fieldId) {
                case 1:
                    if (fieldType === thrift.TType.I32) {
                        const value_1: number = input.readI32();
                        _args.code = value_1;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 2:
                    if (fieldType === thrift.TType.STRING) {
                        const value_2: string = input.readString();
                        _args.msg = value_2;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 3:
                    if (fieldType === thrift.TType.STRUCT) {
                        const value_3: __ROOT_NAMESPACE__.IGetUserAuthorityData = __ROOT_NAMESPACE__.GetUserAuthorityDataCodec.decode(input);
                        _args.data = value_3;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 255:
                    if (fieldType === thrift.TType.STRUCT) {
                        const value_4: __ROOT_NAMESPACE__.IBaseResp = __ROOT_NAMESPACE__.BaseRespCodec.decode(input);
                        _args.BaseResp = value_4;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                default: {
                    input.skip(fieldType);
                }
            }
            input.readFieldEnd();
        }
        input.readStructEnd();
        if (_args.code !== undefined && _args.msg !== undefined) {
            return {
                code: _args.code,
                msg: _args.msg,
                data: _args.data,
                BaseResp: _args.BaseResp
            };
        }
        else {
            throw new thrift.TProtocolException(thrift.TProtocolExceptionType.UNKNOWN, "Unable to read GetUserAuthorityResponse from input");
        }
    }
};
export class GetUserAuthorityResponse extends thrift.StructLike implements IGetUserAuthorityResponse {
    public code: number;
    public msg: string;
    public data?: __ROOT_NAMESPACE__.IGetUserAuthorityData;
    public BaseResp?: __ROOT_NAMESPACE__.IBaseResp;
    public readonly _annotations: thrift.IThriftAnnotations = {};
    public readonly _fieldAnnotations: thrift.IFieldAnnotations = {
        data: {
            'api.body': "data"
        }
    };
    constructor(args: IGetUserAuthorityResponseArgs) {
        super();
        if (args.code != null) {
            const value_5: number = args.code;
            this.code = value_5;
        }
        else {
            throw new thrift.TProtocolException(thrift.TProtocolExceptionType.UNKNOWN, "Required field[code] is unset!");
        }
        if (args.msg != null) {
            const value_6: string = args.msg;
            this.msg = value_6;
        }
        else {
            throw new thrift.TProtocolException(thrift.TProtocolExceptionType.UNKNOWN, "Required field[msg] is unset!");
        }
        if (args.data != null) {
            const value_7: __ROOT_NAMESPACE__.IGetUserAuthorityData = new __ROOT_NAMESPACE__.GetUserAuthorityData(args.data);
            this.data = value_7;
        }
        if (args.BaseResp != null) {
            const value_8: __ROOT_NAMESPACE__.IBaseResp = new __ROOT_NAMESPACE__.BaseResp(args.BaseResp);
            this.BaseResp = value_8;
        }
    }
    public static read(input: thrift.TProtocol): GetUserAuthorityResponse {
        return new GetUserAuthorityResponse(GetUserAuthorityResponseCodec.decode(input));
    }
    public static write(args: IGetUserAuthorityResponseArgs, output: thrift.TProtocol): void {
        return GetUserAuthorityResponseCodec.encode(args, output);
    }
    public write(output: thrift.TProtocol): void {
        return GetUserAuthorityResponseCodec.encode(this, output);
    }
}
