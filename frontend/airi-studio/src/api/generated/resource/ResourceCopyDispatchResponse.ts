/* tslint:disable */
/* eslint-disable */
/*
 * Autogenerated by @creditkarma/thrift-typescript v3.7.6
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
*/
import * as thrift from "@creditkarma/thrift-server-core";
import * as __ROOT_NAMESPACE__ from "./";
export interface IResourceCopyDispatchResponse {
    code?: thrift.Int64;
    msg?: string;
    task_id?: string;
    failed_reasons?: Array<__ROOT_NAMESPACE__.IResourceCopyFailedReason>;
    BaseResp: __ROOT_NAMESPACE__.IBaseResp;
}
export interface IResourceCopyDispatchResponseArgs {
    code?: number | string | thrift.Int64;
    msg?: string;
    task_id?: string;
    failed_reasons?: Array<__ROOT_NAMESPACE__.IResourceCopyFailedReasonArgs>;
    BaseResp: __ROOT_NAMESPACE__.IBaseRespArgs;
}
export const ResourceCopyDispatchResponseCodec: thrift.IStructCodec<IResourceCopyDispatchResponseArgs, IResourceCopyDispatchResponse> = {
    encode(args: IResourceCopyDispatchResponseArgs, output: thrift.TProtocol): void {
        const obj: any = {
            code: (typeof args.code === "number" ? new thrift.Int64(args.code) : typeof args.code === "string" ? thrift.Int64.fromDecimalString(args.code) : args.code),
            msg: args.msg,
            task_id: args.task_id,
            failed_reasons: args.failed_reasons,
            BaseResp: args.BaseResp
        };
        output.writeStructBegin("ResourceCopyDispatchResponse");
        if (obj.code != null) {
            output.writeFieldBegin("code", thrift.TType.I64, 1);
            output.writeI64((typeof obj.code === "number" ? new thrift.Int64(obj.code) : typeof obj.code === "string" ? thrift.Int64.fromDecimalString(obj.code) : obj.code));
            output.writeFieldEnd();
        }
        if (obj.msg != null) {
            output.writeFieldBegin("msg", thrift.TType.STRING, 2);
            output.writeString(obj.msg);
            output.writeFieldEnd();
        }
        if (obj.task_id != null) {
            output.writeFieldBegin("task_id", thrift.TType.STRING, 3);
            output.writeString(obj.task_id);
            output.writeFieldEnd();
        }
        if (obj.failed_reasons != null) {
            output.writeFieldBegin("failed_reasons", thrift.TType.LIST, 4);
            output.writeListBegin(thrift.TType.STRUCT, obj.failed_reasons.length);
            obj.failed_reasons.forEach((value_1: __ROOT_NAMESPACE__.IResourceCopyFailedReasonArgs): void => {
                __ROOT_NAMESPACE__.ResourceCopyFailedReasonCodec.encode(value_1, output);
            });
            output.writeListEnd();
            output.writeFieldEnd();
        }
        if (obj.BaseResp != null) {
            output.writeFieldBegin("BaseResp", thrift.TType.STRUCT, 255);
            __ROOT_NAMESPACE__.BaseRespCodec.encode(obj.BaseResp, output);
            output.writeFieldEnd();
        }
        else {
            throw new thrift.TProtocolException(thrift.TProtocolExceptionType.UNKNOWN, "Required field[BaseResp] is unset!");
        }
        output.writeFieldStop();
        output.writeStructEnd();
        return;
    },
    decode(input: thrift.TProtocol): IResourceCopyDispatchResponse {
        let _args: any = {};
        input.readStructBegin();
        while (true) {
            const ret: thrift.IThriftField = input.readFieldBegin();
            const fieldType: thrift.TType = ret.fieldType;
            const fieldId: number = ret.fieldId;
            if (fieldType === thrift.TType.STOP) {
                break;
            }
            switch (fieldId) {
                case 1:
                    if (fieldType === thrift.TType.I64) {
                        const value_2: thrift.Int64 = input.readI64();
                        _args.code = value_2;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 2:
                    if (fieldType === thrift.TType.STRING) {
                        const value_3: string = input.readString();
                        _args.msg = value_3;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 3:
                    if (fieldType === thrift.TType.STRING) {
                        const value_4: string = input.readString();
                        _args.task_id = value_4;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 4:
                    if (fieldType === thrift.TType.LIST) {
                        const value_5: Array<__ROOT_NAMESPACE__.IResourceCopyFailedReason> = new Array<__ROOT_NAMESPACE__.IResourceCopyFailedReason>();
                        const metadata_1: thrift.IThriftList = input.readListBegin();
                        const size_1: number = metadata_1.size;
                        for (let i_1: number = 0; i_1 < size_1; i_1++) {
                            const value_6: __ROOT_NAMESPACE__.IResourceCopyFailedReason = __ROOT_NAMESPACE__.ResourceCopyFailedReasonCodec.decode(input);
                            value_5.push(value_6);
                        }
                        input.readListEnd();
                        _args.failed_reasons = value_5;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                case 255:
                    if (fieldType === thrift.TType.STRUCT) {
                        const value_7: __ROOT_NAMESPACE__.IBaseResp = __ROOT_NAMESPACE__.BaseRespCodec.decode(input);
                        _args.BaseResp = value_7;
                    }
                    else {
                        input.skip(fieldType);
                    }
                    break;
                default: {
                    input.skip(fieldType);
                }
            }
            input.readFieldEnd();
        }
        input.readStructEnd();
        if (_args.BaseResp !== undefined) {
            return {
                code: _args.code,
                msg: _args.msg,
                task_id: _args.task_id,
                failed_reasons: _args.failed_reasons,
                BaseResp: _args.BaseResp
            };
        }
        else {
            throw new thrift.TProtocolException(thrift.TProtocolExceptionType.UNKNOWN, "Unable to read ResourceCopyDispatchResponse from input");
        }
    }
};
export class ResourceCopyDispatchResponse extends thrift.StructLike implements IResourceCopyDispatchResponse {
    public code?: thrift.Int64;
    public msg?: string;
    public task_id?: string;
    public failed_reasons?: Array<__ROOT_NAMESPACE__.IResourceCopyFailedReason>;
    public BaseResp: __ROOT_NAMESPACE__.IBaseResp;
    public readonly _annotations: thrift.IThriftAnnotations = {};
    public readonly _fieldAnnotations: thrift.IFieldAnnotations = {};
    constructor(args: IResourceCopyDispatchResponseArgs) {
        super();
        if (args.code != null) {
            const value_8: thrift.Int64 = (typeof args.code === "number" ? new thrift.Int64(args.code) : typeof args.code === "string" ? thrift.Int64.fromDecimalString(args.code) : args.code);
            this.code = value_8;
        }
        if (args.msg != null) {
            const value_9: string = args.msg;
            this.msg = value_9;
        }
        if (args.task_id != null) {
            const value_10: string = args.task_id;
            this.task_id = value_10;
        }
        if (args.failed_reasons != null) {
            const value_11: Array<__ROOT_NAMESPACE__.IResourceCopyFailedReason> = new Array<__ROOT_NAMESPACE__.IResourceCopyFailedReason>();
            args.failed_reasons.forEach((value_13: __ROOT_NAMESPACE__.IResourceCopyFailedReasonArgs): void => {
                const value_14: __ROOT_NAMESPACE__.IResourceCopyFailedReason = new __ROOT_NAMESPACE__.ResourceCopyFailedReason(value_13);
                value_11.push(value_14);
            });
            this.failed_reasons = value_11;
        }
        if (args.BaseResp != null) {
            const value_12: __ROOT_NAMESPACE__.IBaseResp = new __ROOT_NAMESPACE__.BaseResp(args.BaseResp);
            this.BaseResp = value_12;
        }
        else {
            throw new thrift.TProtocolException(thrift.TProtocolExceptionType.UNKNOWN, "Required field[BaseResp] is unset!");
        }
    }
    public static read(input: thrift.TProtocol): ResourceCopyDispatchResponse {
        return new ResourceCopyDispatchResponse(ResourceCopyDispatchResponseCodec.decode(input));
    }
    public static write(args: IResourceCopyDispatchResponseArgs, output: thrift.TProtocol): void {
        return ResourceCopyDispatchResponseCodec.encode(args, output);
    }
    public write(output: thrift.TProtocol): void {
        return ResourceCopyDispatchResponseCodec.encode(this, output);
    }
}
