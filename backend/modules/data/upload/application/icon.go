...
import (
	...
	"github.com/kiosk404/airi-go/backend/modules/data/upload/domain/service"
	"github.com/kiosk404/airi-go/backend/modules/data/upload/pkg"
...
func InitService(ctx context.Context, oss storage.Storage, cache cache.Cmdable, db rdb.Provider, idgen idgen.IDGenerator) *UploadService {
	SVC.cache = cache
	SVC.oss = oss
	SVC.UploadSVC = service.NewUploadSVC(db, idgen, oss)
	return SVC
}
...
func (u *UploadService) UploadFileOpen(ctx context.Context, req *bot_open_api.UploadFileOpenRequest) (*bot_open_api.UploadFileOpenResponse, error) {
	...
	if err != nil {
		logs.ErrorX(pkg.ModelName, "parse multipart form data failed, err: %v", err)
		return nil, err
	}
...
func (u *UploadService) GetObjInfoBySessionKey(ctx context.Context, sessionKey string) (*GetObjInfoBySessionKey, error) {
	...
	if isImageUri(objKey) {
		...
		if isSVG(objKey) {
			...
			if err != nil {
				logs.ErrorX(pkg.ModelName, "get svg dimensions failed, err: %v", err)
				// default val
				resp.Width = 100
				resp.Height = 100
				return &resp, nil
			...
			if err != nil {
				logs.ErrorX(pkg.ModelName, "decode image failed, err: %v", err)
				// default val
				resp.Width = 100
				resp.Height = 100
				return &resp, nil
