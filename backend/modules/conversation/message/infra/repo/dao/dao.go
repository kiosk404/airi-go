...
func (dao *MessageDAO) List(ctx context.Context, listMeta *entity.ListMeta) ([]*entity.Message, bool, error) {
	...
	if listMeta.Cursor > 0 {
		msg, err := mq.Where(m.ID.Eq(listMeta.Cursor)).First()
		if err != nil {
...
func (dao *MessageDAO) messageDO2PO(ctx context.Context, msgDo *entity.Message) (*model.Message, error) {
	...
	msgPO := &model.Message{
		...
		SectionID:        ptr.Of(msgDo.SectionID),
		UserID:           msgDo.UserID,
		Role:             string(msgDo.Role),
		ContentType:      string(msgDo.ContentType),
		MessageType:      string(msgDo.MessageType),
		DisplayContent:   ptr.Of(msgDo.DisplayContent),
		Content:          ptr.Of(msgDo.Content),
		BrokenPosition:   ptr.Of(msgDo.Position),
		Status:           int32(entity.MessageStatusAvailable),
		CreatedAt:        time.Now().UnixMilli(),
		UpdatedAt:        time.Now().UnixMilli(),
		ReasoningContent: ptr.Of(msgDo.ReasoningContent),
	}
	...
	if msgDo.ModelContent != "" {
		msgPO.ModelContent = ptr.Of(msgDo.ModelContent)
	} else {
		mc, err := dao.buildModelContent(msgDo)
		if err != nil {
			return nil, err
		}
		msgPO.ModelContent = ptr.Of(mc)
	}
...
func (dao *MessageDAO) batchMessagePO2DO(msgPOs []*model.Message) []*entity.Message {
	return slices.Transform(msgPOs, func(msgPO *model.Message) *entity.Message {
		msgDO := &entity.Message{
			...
			SectionID:        ptr.From(msgPO.SectionID),
			UserID:           msgPO.UserID,
			RunID:            msgPO.RunID,
			...
			Position:         ptr.From(msgPO.BrokenPosition),
			ModelContent:     ptr.From(msgPO.ModelContent),
			Content:          ptr.From(msgPO.Content),
			Status:           message.MessageStatus(msgPO.Status),
			DisplayContent:   ptr.From(msgPO.DisplayContent),
			CreatedAt:        msgPO.CreatedAt,
			UpdatedAt:        msgPO.UpdatedAt,
			ReasoningContent: ptr.From(msgPO.ReasoningContent),
		}

		var ext map[string]string
		err := json.Unmarshal([]byte(ptr.From(msgPO.Ext)), &ext)
		if err == nil {
			msgDO.Ext = ext
		}
