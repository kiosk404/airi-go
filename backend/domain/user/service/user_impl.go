...
func (u *userImpl) Login(ctx context.Context, account, password string) (user *userEntity.User, err error) {
...
func (u *userImpl) Create(ctx context.Context, req *CreateUserRequest) (user *userEntity.User, err error) {
	...
	newUser := &model.User{
		...
		Name:         name,
		UniqueName:   u.getUniqueNameFormEmail(ctx, req.Account),
		Account:      req.Account,
...
func (u *userImpl) getUniqueNameFormEmail(ctx context.Context, email string) string {
	...
	if err != nil {
		logs.Warn("check unique name exist failed: %v", err)
		return email
	}

	if exist {
		logs.Warn("unique name %s already exist", username)
